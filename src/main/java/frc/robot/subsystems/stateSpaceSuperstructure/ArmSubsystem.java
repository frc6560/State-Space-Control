...
public class ArmSubsystem extends SubsystemBase {
  ...
  /** True only when the arm is both close to its goal and nearly stationary. */
  public boolean atGoal() {
    return Math.abs(getAngleRad() - target.angleRad)
            <= Constants.Coordination.ARM_ANGLE_TOLERANCE_RAD
        && Math.abs(getVelocityRadPerSec())
            <= Constants.Coordination.ARM_VELOCITY_TOLERANCE_RAD_PER_SECOND;
  }
